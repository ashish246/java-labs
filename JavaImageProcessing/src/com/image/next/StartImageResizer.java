...
public class StartImageResizer
{   
    /**
                 * The properties file
                 */
    ...
    */  
    public static void main(String[] args)
    {
        ...
        try
        {
            properties.load(new FileReader(new File("D:/videocon/workspace/ImageResizer/javasource/com/eperium/images/image_resizing.properties")));
        }
                        ...
                        */
                        ...
                        try
                        {
                            ...
                            for(int t=0;t<resize_sizes.length;t++)
                            {
                                ...
                                System.out.println("Operation on this particular string" + resize_sizes[t].toString());
                                ...
                                String resize_type = properties.getProperty(resize_sizes[t] + ".resize.type");
                                //String resizeSizeFlag = resize_sizes[t].substring(0, 1).toLowerCase();
                                String targetFileName= fileNames[i].toLowerCase();
                               
                                targetFileName = targetFileName.substring(0,targetFileName.indexOf(".jpg"))+".jpg";
                                File targetFile = new File(properties.getProperty("base_image_dir") + "/" + resize_sizes[t] + "/" + targetFileName);
                                /** only resize the file if the targetfile does not exist or is older then the original file*/
                                if((targetFile.exists() && targetFile.lastModified() < originalFile.lastModified()) || (!targetFile.exists()))
                                {
                                    logEvent(("Resizing file " + fileNames[i] + " using width: " + width + ", height: " + height + " and resize_type: " + resize_type), logFileWriter);
                                    logFileWriter.flush();
									int type = (original.getTransparency() == Transparency.OPAQUE) ? BufferedImage.TYPE_INT_RGB : BufferedImage.TYPE_INT_ARGB;
                                    BufferedImage resized = getScaledInstance(original, width, height, RenderingHints.VALUE_INTERPOLATION_BILINEAR, resize_type, logFileWriter, fileNames[i]);   
								    ...
								    g.dispose();  
                                    if(finalImg != null){
										ImageIO.write(finalImg, "jpg", targetFile);										
									}
									/*if(resized != null){
                                        ...
                                        ImageIO.write(resized, "jpg", targetFile);
                                         ...
                                         logEvent("Skipped file " + fileNames[i] + " null returned by getScaledInstance", logFileWriter);   
                                         logFileWriter.flush();   
                                         error = true;
                                    }
                                }
